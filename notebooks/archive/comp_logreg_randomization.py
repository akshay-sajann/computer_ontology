...
alldesc_train = pd.read_csv(alldesc_train_path)
computer_train = pd.read_csv(computer_train_path)

alldesc_test = pd.read_csv(alldesc_test_path)
computer_test = pd.read_csv(computer_test_path)

# Setting CIDs as indices
alldesc_train.set_index('CID', inplace=True)
computer_train.set_index('CID', inplace=True)
alldesc_test.set_index('CID', inplace=True)
computer_test.set_index('CID', inplace=True)
...
X, train_y = x_y_split(computer_train)
X, test_y = x_y_split(computer_test)

clf = OneVsRestClassifier(LogisticRegression(**logreg_hyperparams))
...
labels_df = pd.read_excel(computer_path, index_col=0)
...
with open(comp_rand_logreg_path, 'wb') as fp:
